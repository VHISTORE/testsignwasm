...
self.onmessage = async function(e) {
    ...
    try {
        self.postMessage({ type: 'status', msg: '1/4 Initializing Native WASM Engine...' });
        ...
        self.postMessage({ type: 'status', msg: '3/4 Native Signing & Packaging...' });
        ...
        const args = [
            ...
            '-o', 'signed.ipa',
            '-f',
            '-z', '9', // Отличная компрессия силами самого zsign
            '-b', 'app.raspberry9732.test9663'
        ];
        
        zsignModule.callMain(args);

        self.postMessage({ type: 'status', msg: '4/4 Extracting perfect signed file...' });
